...
/**
 * `mini.ai`: `a` and `i` text objects that search for their delimiter instead of requiring the
 * caret to be inside it.
 *
 * `ci(` from anywhere on the line changes the next parenthesised group, where Vim's own `ci(`
 * needs the caret already inside one. That is the whole difference, and it is the reason people
 * install it.
 *
 * Ported by moving the body to the engine and registering it with `@VimPlugin`; the plugin keeps a
 * two-line adapter. See `ParagraphMotionNewApi` for the shape and why.
 */
@VimPlugin(name = MINI_AI)
...
public const val MINI_AI: String = "mini-ai"
