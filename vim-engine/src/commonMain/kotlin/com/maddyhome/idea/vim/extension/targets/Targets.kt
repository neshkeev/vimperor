...
/** A fresh [Targets] per enable, because it carries the last target it produced. */
@VimPlugin(name = TARGETS)
public fun VimInitApi.init(): Unit = Targets().register()

/** Public because the plugin's extension-point adapter names it too. */
public const val TARGETS: String = "targets"
 ...
 *
 * One line, the same one `textobj-user` cost:
 * `(caret as IjVimCaret).caret.moveToInlayAwareOffset(...)`, where
 * `moveToInlayAwareOffset` is a member of the engine's own `VimCaret` and the cast reached
 * IntelliJ's `Caret` to call what the engine already offered. 808 lines, and the entire
 * IntelliJ debt was that cast; `KeyHandler`, `SelectionVimListenerSuppressor` and
 * `vimSetSelection` all sound host-shaped and are all `commonMain`.
 *
 * That the class survives as a class matters: [lastTarget] is what makes `vii` grow the
 * selection outward on a second press, and it has to outlive one keystroke.
 */
public class Targets {

  private val owner = MappingOwner.Plugin.get(TARGETS)
