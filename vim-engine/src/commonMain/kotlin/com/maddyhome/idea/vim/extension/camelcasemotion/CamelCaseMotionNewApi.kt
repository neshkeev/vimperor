...
/**
 * `CamelCaseMotion`: `,w`, `,b`, `,e` and the matching text objects, which move by the parts of an
 * identifier instead of by whitespace.
 *
 * `w` on `getUserName` jumps past the whole thing; `,w` stops at `User`, then `Name`. Same for
 * `snake_case`, which is the other half of what it is for.
 *
 * This is the extension that `VimExtensionFacade` was blocking. Its body was portable all along -
 * it registers mappings and nothing else - but `putExtensionHandlerMapping` lived in the plugin,
 * so it could not follow. Moving the facade to the engine is what let it move, and it is why that
 * refactor was worth doing before porting any more of these one at a time.
 */
@VimPlugin(name = CAMEL_CASE_MOTION)
public fun VimInitApi.init() {
  // `VimExtension.getOwner()` supplied this when the extension was a class; the thin API has
  // no class, and the owner is derived from the name the same way the loader derives it.
  val owner = MappingOwner.Plugin.get(CAMEL_CASE_MOTION)
  val key: String? = getVariable<String>("g:camelcasemotion_key")
...
public const val CAMEL_CASE_MOTION: String = "CamelCaseMotion"
