 ...
 */
@VimPlugin(name = ABOLISH)
public fun VimInitApi.init(): Unit = Abolish().register()

/** Public because the plugin's extension-point adapter names it too. */
public const val ABOLISH: String = "abolish"

public class Abolish {

  private val owner = MappingOwner.Plugin.get(ABOLISH)
  ...
  private fun registerUserDefinedCoercions() {
    val dict =
      injector.variableService.getGlobalVariableValue(USER_COERCIONS_VARIABLE) as? VimDictionary ?: return
  ...
  internal companion object {
    const val OPERATOR_FUNC = "AbolishCoerce"
...
private class CoercionOperatorTrigger(private val style: CaseStyle) : ExtensionHandler {
  ...
  override fun execute(editor: VimEditor, context: ExecutionContext, operatorArguments: OperatorArguments) {
    ...
    injector.globalOptions().operatorfunc = Abolish.OPERATOR_FUNC
