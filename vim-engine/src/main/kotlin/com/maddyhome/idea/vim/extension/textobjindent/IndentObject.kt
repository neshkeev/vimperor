 ...
 *
 * ## What moving it to the engine cost
 *
 * Two lines and a loop. `getRange` read the document as
 * `(editor as IjVimEditor).editor.document.charsSequence` and the caret as
 * `(caret as IjVimCaret).caret.offset`, which are `editor.text()` and `caret.offset`; and `execute`
 * walked the carets through IntelliJ's `CaretModel.runForEachCaret`, wrapping each native `Caret`
 * back into an `IjVimCaret` to hand to engine code that had just been given one - which is
 * `editor.nativeCarets().forEach`. Nothing else in 279 lines was IntelliJ-shaped.
 */
@VimPlugin(name = TEXT_OBJ_INDENT)
public fun VimInitApi.init(): Unit = registerIndentObject()

/** Public because the plugin's extension-point adapter names it too. */
public const val TEXT_OBJ_INDENT: String = "textobj-indent"

/** `ai`, `aI` and `ii`, plus the `<Plug>` names they are mapped through so they stay remappable. */
public fun registerIndentObject() {
  val owner = MappingOwner.Plugin.get(TEXT_OBJ_INDENT)
  putExtensionHandlerMapping(
    MappingMode.XO, injector.parser.parseKeys("<Plug>textobj-indent-ai"), owner,
  ...
  putExtensionHandlerMapping(
    MappingMode.XO, injector.parser.parseKeys("<Plug>textobj-indent-ii"), owner,
