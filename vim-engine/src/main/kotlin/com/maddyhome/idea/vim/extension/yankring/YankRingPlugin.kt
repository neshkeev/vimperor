...
public const val PLUGIN_NAME: String = "yankring"
...
/**
 * Called on `set noyankring`.
 *
 * The engine's loader tears down mappings and listeners by owner, and everything else the extension
 * registered has to be undone here - commands included, or `:YRShow` keeps working after the user
 * has turned the plugin off. The register listener is one of those: it goes on
 * `listenersNotifier.registerListeners` directly rather than through the thin API's listener scope,
 * so nothing removes it by owner either.
 *
 ...
 */
public fun disposeYankRing() {
  injector.listenersNotifier.registerListeners.remove(YankRingRecorder)
  injector.keyGroup.removeKeyMapping(MappingOwner.Plugin.get(PLUGIN_NAME))
