 ...
 * `:help filename-modifiers`). Neither host has a `:cd`, so the buffer name comes from
 * [com.maddyhome.idea.vim.api.VimFile.bufferName] - the same value `:file` and the `%` register report, which is
 * relative to the content root in IdeaVim and to the workspace folder here, and absolute for a file outside it.
 */
