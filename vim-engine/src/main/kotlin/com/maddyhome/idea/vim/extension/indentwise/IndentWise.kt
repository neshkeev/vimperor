 ...
 *
 * Motions that navigate by the indentation level of lines: `[-` and `]-` to the nearest line
 * indented less, `[+` and `]+` to one indented more, `[=` and `]=` to the next at the same level,
 * `[%` and `]%` to the boundaries of the current block. In a language whose blocks *are*
 * indentation, that is structural movement without a parser.
 *
 * The last extension `VimExtensionFacade` was blocking. Its body was always pure engine code; the
 * only thing it could not reach was the function that registers a mapping.
 */
@VimPlugin(name = INDENTWISE)
public fun VimInitApi.init(): Unit = IndentWise().register()

/** Public because the plugin's extension-point adapter names it too. */
public const val INDENTWISE: String = "indentwise"
public class IndentWise {
  ...
  /** Registers the motions and their default keys. */
  public fun register() {
    val owner = MappingOwner.Plugin.get(INDENTWISE)
