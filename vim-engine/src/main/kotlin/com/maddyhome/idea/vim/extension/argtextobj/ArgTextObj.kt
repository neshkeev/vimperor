...
import com.maddyhome.idea.vim.helper.isIdentifierStart
...
@VimPlugin(name = ARG_TEXT_OBJ)
public fun VimInitApi.init(): Unit = registerArgTextObj()

/** Public because the plugin's extension-point adapter names it too. */
public const val ARG_TEXT_OBJ: String = "argtextobj"

/** `ia` and `aa`, through `<Plug>` names so a user can put them somewhere else. */
public fun registerArgTextObj() {
  val owner = MappingOwner.Plugin.get(ARG_TEXT_OBJ)
...
private class BracketPairs(openBrackets: String, closeBrackets: String) {
  ...
  init {
    StrictMode.assert(openBrackets.length == closeBrackets.length, "argtextobj: bracket pairs must match up")
  ...
  fun matchingBracket(ch: Char): Char {
    ...
    if (idx != -1) {
      ...
      StrictMode.assert(isOpenBracket(ch.code), "argtextobj: matchingBracket asked about a non-bracket")
    ...
    override fun getRange(
      ...
      if (bracketPairsVar != null) {
        try {
          ...
          val message =
            injector.messages.message("argtextobj.error.invalid.value.of.g.argtextobj.pairs.0", parseException.message!!)
          injector.messages.showStatusBarMessage(null, message)
          injector.messages.indicateError()
          return null
        }
      }
      val finder = ArgBoundsFinder(editor, bracketPairs)
      var pos = caret.offset

      for (i in 0..<count) {
        if (!finder.findBoundsAt(pos)) {
          injector.messages.showStatusBarMessage(null, finder.errorMessage())
          injector.messages.indicateError()
          return null
        }
        if (i + 1 < count) {
 ...
 * position
 */
private class ArgBoundsFinder(private val editor: VimEditor, private val brackets: BracketPairs) {
  private val text: CharSequence = editor.text()
  ...
  fun adjustForInner() {
    ++leftBound
    while (leftBound < rightBound && isVimWhitespace(getCharAt(leftBound))) {
      ++leftBound
    }
  }
  ...
  fun extendTillNext() {
    if (rightBound + 1 < rightBracket && getCharAt(rightBound) == ',') {
      ++rightBound
      while (rightBound + 1 < rightBracket && isVimWhitespace(getCharAt(rightBound))) {
        ++rightBound
      }
    }
  }
  ...
  val isIdentPreceding: Boolean
    get() {
      var i = leftBound - 1
      val idEnd = i
      while (i >= 0 && isIdentifierPart(getCharAt(i))) {
        --i
      }
      return (idEnd - i) > 0 && isIdentifierStart(getCharAt(i + 1))
    }
  ...
  val outOfQuotedText: Unit
    ...
    get() {
      ...
      val lineNo = editor.offsetToBufferPosition(leftBound).line
      val lineStartOffset = editor.getLineStartOffset(lineNo)
      val lineEndOffset = editor.getLineEndOffset(lineNo)
  ...
  fun getCharAt(logicalOffset: Int): Char {
    StrictMode.assert(logicalOffset < text.length, "argtextobj: read past the end of the buffer")
    return text[logicalOffset]
  }

  fun skipQuotedTextForward(start: Int, end: Int): Int {
    StrictMode.assert(start < end, "argtextobj: forward scan given a backwards range")
  ...
  fun skipQuotedTextBackward(start: Int, end: Int): Int {
    StrictMode.assert(start > end, "argtextobj: backward scan given a forwards range")
    val quoteChar = getCharAt(start)
    var i = start - 1
  ...
  fun leftLimit(pos: Int): Int {
    ...
    val lineNo = editor.offsetToBufferPosition(pos).line
    val lineOffsetLimit = editor.getLineStartOffset(max(0, lineNo - MAX_SEARCH_LINES))
  ...
  fun rightLimit(pos: Int): Int {
    ...
    val lineNo = editor.offsetToBufferPosition(pos).line
    val lineOffsetLimit = editor.getLineEndOffset(min(editor.lineCount() - 1, lineNo + MAX_SEARCH_LINES))
  ...
  fun skipSexp(start: Int, end: Int, dir: SexpDirection): Int {
    ...
    StrictMode.assert(dir.isOpenBracket(lastChar), "argtextobj: skipSexp did not start on a bracket")
    // `addFirst`/`removeFirst`/`last()` are Java's `Deque.push`/`pop`/`getLast()`, which is what
    // this was written against. Note that `getLast` is the *bottom* of a stack pushed with `push`,
    // not the top - preserved as it was, because the plugin's own tests pin the behaviour.
    val bracketStack = ArrayDeque<Char?>()
    bracketStack.addFirst(lastChar)
    ...
    while (!bracketStack.isEmpty() && i != end) {
      ...
      if (dir.isOpenBracket(ch)) {
        ...
        if (dir.isCloseBracket(ch)) {
          if (bracketStack.last() == brackets.matchingBracket(ch)) {
            bracketStack.removeFirst()
          } else {
            if (brackets.getBracketPrio(ch) < brackets.getBracketPrio(bracketStack.last()!!)) {
              // (<...) ->  (...)
              bracketStack.removeFirst()
...
private object Util {
  ...
  fun bracketPairsVariable(): String? {
    val value: Any? = injector.variableService.getGlobalVariableValue("argtextobj_pairs")
