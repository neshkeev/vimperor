 ...
 * modifiers that work on a plain string. Resolving `%` itself and `:p` needs a buffer with a name, so those are
 * covered by the host tests instead - see `FileNameRegisterTest` and `PercentExpansionTest`.
 *
 * Upstream reaches for Mockito here. This tree compiles for JS as well as the JVM, where there is no Mockito and no
 * JUnit either, so the editor is the headless host's own `TestVimEditor` - which none of these modifiers looks at:
 * `:t`, `:h`, `:r` and `:e` are string operations, and only `:p` asks the host anything.
 */
class PathCompletionArgumentParserTest {

  // `TestVimEditor` reads `injector` while it is being built, so there has to be one before the
  // first editor rather than only before the first call that uses it.
  @BeforeTest
  ...
  /** None of these modifiers reads the editor; only `:p` does, and that needs a host. */
  private fun mockEditor(): VimEditor = TestVimEditor("", emptyList())
}
