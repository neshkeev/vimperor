...
class JvmOnlyTestsTest {
  ...
  fun `test every JVM-only engine test is listed with a reason`() {
    ...
    val directory = root!!.resolve("vim-engine/jvm/src/test/kotlin")
   ...
   * above would be satisfied by moving everything into vim-engine/jvm and writing a reason for
   * each.
   */
  @Test
  fun `test most of the engine's tests run on both targets`() {
    ...
    fun count(tree: String) = Files.walk(root.resolve("vim-engine/$tree")).use { paths ->
      paths.filter { it.extension == "kt" }.count()
    }
    val common = count("src/test/kotlin")
    val jvmOnly = count("jvm/src/test/kotlin")
