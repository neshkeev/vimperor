...
// Walk up to whichever directory holds package.json rather than counting `..` segments. This
// script runs from two places - the source tree and the packaged copy - and it used to count
// them, which broke silently the first time the directory moved a level deeper.
const extensionRoot = (() => {
  let directory = __dirname
  while (!fs.existsSync(path.join(directory, 'package.json'))) {
    const parent = path.dirname(directory)
    if (parent === directory) throw new Error(`no package.json above ${__dirname}`)
    directory = parent
  }
  return directory
})()
